...
data <- read.table(gzfile("estat_prc_rem_cs.tsv.gz"), header = TRUE, sep = "\t") %>% 
  ...
  select(c('country', '2013', '2014', '2014', '2015', '2016', '2017', '2018', '2019', '2020'))
...
data <- data %>% mutate(`2013` = as.numeric(`2013`)) %>% 
  ...
  mutate(`Remuneration indexes` = rowMeans(select(., `2013`, `2014`, `2015`, `2016`), na.rm = TRUE)) %>% 
...
wykres <- ggplot() +
  ...
  labs(title = "Ewolucja Wynagrodzeń Służby Publicznej w Europie w latach 2013-2020",
       subtitle = 
