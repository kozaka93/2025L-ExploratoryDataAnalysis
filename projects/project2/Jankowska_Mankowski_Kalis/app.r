...
# 
# 
# write.csv(ts_coords, "ts_df.csv", row.names = FALSE) # - nowy plik, który
# #został ponownie wczytany na początku (została zmieniona nazwa)
    ...
    filtered_ts() %>%
      ...
      summarise(
        details = paste0(
          ...
          "🎟️ ", format(tickets, big.mark = " ", scientific = FALSE),
          "<br>"
        ) %>% paste(collapse = ""),
